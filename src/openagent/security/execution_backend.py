...
class ContainerSandboxBackend:
    ...
    def execute(
        ...
        *,
        ...
        with (
            ...
            original = {
                path.relative_to(source) for path in SafeWorkspaceWalker(source).iter_files()
            }
            ...
            create = self._control(
                [
                    ...
                    "256",
                    "--tmpfs",
                    "/workspace:rw,size=1g,mode=0700",
                    "--tmpfs",
    ...
    def _sync_back(self, output: Path, original: set[Path]) -> None:
        output_walker = SafeWorkspaceWalker(output)
        final_files = {path.relative_to(output) for path in output_walker.iter_files()}
        workspace_walker = SafeWorkspaceWalker(self.workspace)
        for relative in sorted(original - final_files):
            try:
                target = workspace_walker.resolve(relative)
                info = target.lstat()
                if stat.S_ISREG(info.st_mode):
                    target.unlink()
            except (FileNotFoundError, UnsafeWorkspacePath, OSError):
                continue
        for relative in sorted(final_files):
            workspace_walker.write_bytes(relative, output_walker.read_bytes(relative), mode=0o600)
