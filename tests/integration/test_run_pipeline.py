...
async def test_full_api_run_produces_bundle(app: OpenAgentApp, httpx_mock: HTTPXMock):
    ...
    app.agents.create(
        ...
        provider="testco", model="test-model", tags=["coder"], permission_profile="safe-edit",
    )
    ...
    httpx_mock.add_response(content=_sse(
        {"id": "c1", "choices": [{"delta": {"tool_calls": [
            {"index": 0, "id": "call_1", "function": {
                "name": "apply_patch",
                "arguments": json.dumps({"path": "main.py", "old_string": "value = 1",
                                          "new_string": "value = 2"})}}]}}]},
    ...
    for name in ("request.json", "status.json", "events.jsonl", "output.md", "result.json",
                 "changes.diff", "tests.json", "handoff.md"):
...
async def test_secrets_never_in_artifacts(app: OpenAgentApp, httpx_mock: HTTPXMock):
    ...
    httpx_mock.add_response(content=_sse(
        {"id": "c", "choices": [{"delta": {"content": "leaking OPENAI_API_KEY=sk-abcd1234567890EFGHIJ now"}}]},
    ), headers={"content-type": "text/event-stream"})
