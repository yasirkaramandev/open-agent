...
def test_read_and_write_roundtrip(tmp_path: Path):
    ...
    patched = execu.execute(ToolCall(
        id="2", name="apply_patch",
...
def test_read_only_profile_blocks_writes(tmp_path: Path):
    ...
    result = execu.execute(ToolCall(
        id="1", name="write_file", arguments={"path": "f.txt", "content": "y"},
    ))
    assert not result.ok
...
def test_denied_command_not_run(tmp_path: Path):
    execu = ToolExecutor(make_ctx(tmp_path))
    result = execu.execute(ToolCall(
        id="1", name="run_command", arguments={"command": "git push"},
    ))
    assert not result.ok
...
def test_run_command_executes(tmp_path: Path):
    execu = ToolExecutor(make_ctx(tmp_path))
    result = execu.execute(ToolCall(
        id="1", name="run_command", arguments={"command": "echo hello-openagent"},
    ))
    assert result.ok
